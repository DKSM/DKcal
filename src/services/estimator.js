...
async function estimateFromImage(imageBase64, unit = '100g', name = '') {
  ...
  const response = await fetch('https://api.groq.com/openai/v1/chat/completions', {
    ...
    body: JSON.stringify({
      ...
      messages: [
        { role: 'system', content: 'Tu es un nutritionniste expert en lecture d\'étiquettes nutritionnelles et en estimation visuelle de plats. Règles strictes :\n- Si l\'image montre une étiquette nutritionnelle, lis les valeurs exactes.\n- Si l\'image montre un plat ou aliment, estime les valeurs.\n- Vérifie la cohérence : kcal ≈ protein×4 + fat×9 + carbs×4.\nRéponds UNIQUEMENT avec {"kcal":X,"protein":X,"fat":X,"carbs":X} — rien d\'autre.' },
        {
          role: 'user',
          content: [
...
module.exports = { estimateNutrition, estimateFromImage };
